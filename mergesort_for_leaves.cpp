...
void merge_for_leaves(Apontador Floresta,int left,int meio,int right){
	...
	while(i<n1 && j<n2){
		if(L[i].Reg.folhas <= R[j].Reg.folhas){
			Floresta[k] = L[i];
			i++;
		}
		else{
...
void mergesort_for_leaves(Apontador Floresta,int left, int right){
	if(left<right){
		...
		mergesort_for_leaves(Floresta,left,meio);
		mergesort_for_leaves(Floresta,meio+1,right);
		merge_for_leaves(Floresta,left,meio,right);
	}
}
